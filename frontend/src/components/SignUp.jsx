...
function SignUp() {
    ...
    const signup = async () => {
        ...
        if(!emailPattern.test(email)){
            ...
            setAlertText('Please Enter Correct Email')
            setShowAlert(true)
            setTimeout(() => {
                setShowAlert(false)
            }, 10000);
        ...
        if(username.length<1){
            setAlertHeader('Invalid Input!')
            setAlertText('Please Enter Username')
            setShowAlert(true)
            setTimeout(() => {
                setShowAlert(false)
            }, 10000);
        ...
        if(password.length<8){
            setAlertHeader('Invalid Input!')
            setAlertText('Password must be 8 characters long')
            setShowAlert(true)
            setTimeout(() => {
                setShowAlert(false)
            }, 10000);
        ...
        if(fetchUrl.status == 400){
            ...
            setAlertText('User Created Successfully! Redirecting to login Page...')
            setShowAlert(true)
            setTimeout(() => {
                navigate('/login')
            }, 5000);
        ...
        <>
            <div className="container flex h-[704px] bg-[#C3C9EF]">
                ...
                <div className="w-140 h-160 bg-white my-8 flex flex-col items-center rounded-[8px] mx-12">
                    ...
                    <div className="email flex flex-col mb-8 inter-font font-semibold">
                        <label htmlFor="email">Email:</label>
                        <input type="email" name="email" id="email" className="bg-[#C3C9EF] px-1 outline-[#C3C9EF] text-lg focus:bg-[#031273] ease-in-out duration-100 focus:text-white rounded-[8px] w-124 h-12" />
                    </div>
                    ...
                    <div className="category flex flex-col mb-8 inter-font font-semibold">
                        <label htmlFor="category">Category:</label>
                        <input type='text' name="category" id="category" className="bg-[#C3C9EF] px-1 outline-[#C3C9EF] text-lg focus:bg-[#031273] ease-in-out duration-100 focus:text-white rounded-[8px] w-124 h-12" />
                    </div>
                    <div className="container flex mb-8 gap-x-2 items-center w-124">
                        <p className="inter-font font-semibold text-lg">Already on Qweegly?</p><Link to={'/login'}><p className="roboto-font italic text-lg text-[#0504AA] underline cursor-pointer">Click to login!</p></Link>
                    </div>
                    <div className="w-40 h-16 rounded-[8px] bg-[#031273] hover:bg-[#020936] ease-in-out duration-100 text-white text-xl cursor-pointer flex justify-center items-center roboto-font font-bold" onClick={signup}>Sign-up</div>
                </div>
